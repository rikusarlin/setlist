...
import { updatePiece, analyzeContents, clearAnalysis, transposeDown, transposeUp } from '../reducers/analyzeReducer'
...
export const NewPieceNoHistory = (props) => {
  ...
  const confirmDelete = () => {
    if(window.confirm(`Remove ${props.analyzedPiece.title} by ${props.analyzedPiece.artist}?`)){
      deletePiece()
    }
  }
  ...
  const editContents = async () => {
    const rowContents = props.analyzedPiece.pages.map(page => page.rows.map(row => row.contents).join('\n'))
  ...
  const editOrAnalyze = props.analyzedPiece !== null ?
    <div>
      <button onClick={updateContents} data-cy="update" className="col-sm-1 mr-2 btn btn-primary">update</button>
      <button onClick={editContents} data-cy="edit" className="col-sm-1 mr-2 btn btn-primary">edit</button>
      ...
      <button onClick={confirmDelete} data-cy="deletePiece" className="col-sm-2 mr-2 btn btn-danger">delete</button>
    </div>
    :
  ...
  return (
    <div>
      ...
      <div className="form-group">
        {editOrAnalyze}
        <button onClick={returnToPieces} data-cy="cancel" className="col-sm-1 mr-2 my-2 btn btn-primary">back</button>
      </div>
    </div>
  )
}
...
const mapDispatchToProps = {
  updatePiece, showInfo, showError, analyzeContents, clearAnalysis, transposeDown, transposeUp, deletePiece
}
